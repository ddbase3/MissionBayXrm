...
final class XrmTextMetaParserAgentResource extends AbstractAgentResource implements IAgentContentParser {

	public static function getName(): string {
		return 'xrmtextmetaparseragentresource';
	}

	public function getDescription(): string {
		return 'Adds type-specific metadata for XRM entries with type_alias=text (mime).';
	}
	...
	public function supports(mixed $item): bool {
		...
		return $this->detectTypeAliasFromItem($item) === 'text';
	}

	public function parse(mixed $item): AgentParsedContent {
		if (!$item instanceof AgentContentItem) {
			throw new \InvalidArgumentException('XrmTextMetaParser expects AgentContentItem.');
		}
		...
		$mime = $this->asTrimmedString($payload['mime'] ?? null);
		if ($mime !== '') {
			$textMeta['mime'] = $mime;
		}

		if (!empty($textMeta)) {
			$existing = $meta['text'] ?? null;
			if (!is_array($existing)) {
				$existing = [];
			}

			// parser wins on conflicts
			$meta['text'] = array_merge($existing, $textMeta);
		}
