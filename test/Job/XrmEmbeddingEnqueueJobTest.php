...
use Base3\Test\State\StateStoreStub;
...
final class XrmEmbeddingEnqueueJobTest extends TestCase {
	...
	private function makeStateMock(): IStateStore {
		return new StateStoreStub();
	}
	...
	public function testGoSkipsWhenMinIntervalNotReached(): void {
		...
		// loadCheckpointFromState: last_run_at = now => should skip
		$state->set('missionbay.xrm.embedding.sysentry.last_changed', '2026-01-01 00:00:00');
		$state->set('missionbay.xrm.embedding.sysentry.last_run_at', date('Y-m-d H:i:s'));
	...
	public function testGoEnqueuesChangedAndDeletesAndUpdatesCheckpoint(): void {
		...
		// singleQuery: delete job id lookup only
		...
		// optional sanity: job should touch checkpoint keys
		$this->assertTrue($state->has('missionbay.xrm.embedding.sysentry.last_run_at'));
	}
	...
	public function testNormalizeCollectionKeyFallsBackToDefaultWhenTypeAliasMissing(): void {
		...
		$db->expects($this->atLeastOnce())
			...
			->with($this->callback(function (string $sql): bool {
				...
				// allow DDL
				if (str_contains($sqlLower, 'create table if not exists')) return true;

				// seen upsert
				if (str_contains($sqlLower, 'insert into base3_embedding_seen')) return true;
				...
				// supersede update
				if (str_contains($sqlLower, 'update base3_embedding_job') && str_contains($sqlLower, "set state = 'superseded'")) return true;
	...
	public function testEnqueueDeletesUsesLastSeenCollectionKeyAndSetsDeleteJobId(): void {
		...
		$db->expects($this->atLeastOnce())
			...
			->with($this->callback(function (string $sql): bool {
				...
				// required signals:
				if (str_contains($sqlLower, 'update base3_embedding_seen') && str_contains($sqlLower, 'set missing_since')) return true;
				if (str_contains($sqlLower, 'insert ignore into base3_embedding_job') && str_contains($sqlLower, "'delete'") && str_contains($sqlLower, "'xrm'")) return true;
				if (str_contains($sqlLower, 'update base3_embedding_seen') && str_contains($sqlLower, 'set delete_job_id = 99')) return true;
